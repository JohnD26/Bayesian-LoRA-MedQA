...
def add_experiment_args(parser: ArgumentParser) -> None:
    ...
    # Uncertainty-based deferral arguments
    parser.add_argument(
        "--enable-deferral",
        action="store_true",
        help="Enable uncertainty-based deferral mechanism during evaluation."
    )
    parser.add_argument(
        "--deferral-threshold",
        type=float,
        default=0.1,
        help="Uncertainty threshold above which predictions are deferred. "
             "Typical range: 0.05-0.2. Higher = more conservative."
    )
    parser.add_argument(
        "--deferral-metric",
        type=str,
        default="max_std",
        choices=["max_std", "mean_std", "entropy", "bald"],
        help="Uncertainty metric for deferral decision. "
             "'max_std': maximum class std deviation (recommended), "
             "'mean_std': average std across classes, "
    ...
    parser.add_argument(
        ...
        type=str,
        default="exclude",
        choices=["exclude", "always_predict"],
        help="How to handle deferred predictions. "
             "'exclude': skip from accuracy calculation (recommended for medical AI), "
             "'always_predict': predict anyway but log as uncertain."
    )
    parser.add_argument(
        "--deferral-log-path",
        type=str,
        default=None,
        help="Path to save deferred questions in JSONL format. "
             "If None, uses checkpoints/{modelwrapper}/{model}/{dataset}/deferred_samples.jsonl"
    )
    parser.add_argument(
        "--deferral-log-to-wandb",
        action="store_true",
        help="Also log deferral statistics to WandB (deferral rate, uncertainty distribution)."
    )
...
def add_management_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--wandb-entity", 
        type=str, 
        default="jonathandomingue15-university-of-ottawa", 
