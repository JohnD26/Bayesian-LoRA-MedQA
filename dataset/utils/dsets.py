...
class MedMCQADataset(ClassificationDataset):
    def __init__(
        self,
        tokenizer: AutoTokenizer,
        add_space: bool = True,
        ...
        dset = load_dataset("openlifescienceai/medmcqa")
        prompt = """Answer the medical question by selecting the correct option.
...
Answer:"""
    ...
    def clm_collate_fn(self, batch):
        prompts = self._format_prompts(batch)
        prompts = self._tokenize_prompts(prompts)
        classes = t.tensor([e["cop"] for e in batch])
        targets = t.cat([self.label2target[c.item()] for c in classes])

        # Add metadata for deferral logging
        metadata = [{
            "question": e["question"],
            "options": {"A": e["opa"], "B": e["opb"], "C": e["opc"], "D": e["opd"]},
            "answer_idx": chr(ord("A") + e["cop"]),  # Convert 0,1,2,3 to A,B,C,D
        } for e in batch]

        return prompts, classes, targets, metadata
...
class USMLEDataset(ClassificationDataset):
    def __init__(
        self,
        tokenizer: AutoTokenizer,
        add_space: bool = True,
        ...
        dset = load_dataset("GBaker/MedQA-USMLE-4-options")

        # Handle missing validation split by using test split for validation
...
Answer:"""
    ...
    def clm_collate_fn(self, batch):
        ...
        classes = t.tensor([ord(e["answer_idx"]) - ord("A") for e in batch])
        targets = t.cat([self.label2target[c.item()] for c in classes])

        # Add metadata for deferral logging
        metadata = [{
            "question": e["question"],
            "options": e["options"],  # Already {"A": "...", "B": "...", "C": "...", "D": "..."}
            "answer_idx": e["answer_idx"],  # Already "A", "B", "C", or "D"
        } for e in batch]
...
class MedicalGeneticsDataset(ClassificationDataset):
    def __init__(
        self,
        tokenizer: AutoTokenizer,
        add_space: bool = True,
        ...
        dset = load_dataset("openlifescienceai/mmlu_medical_genetics")

        # Handle MMLU dataset splits - use 'dev' as train, 'validation' as validation, 'test' as test
        if "train" not in dset:
            dset["train"] = dset["dev"]  # Use dev split for training

        prompt = """Answer the following medical genetics question by selecting the correct option.
...
Answer:"""
    ...
    def clm_collate_fn(self, batch):
        ...
        classes = t.tensor([ord(e["data"]["Correct Option"]) - ord("A") for e in batch])
        targets = t.cat([self.label2target[c.item()] for c in classes])
        return prompts, classes, targets
...
class ProfessionalMedicineDataset(ClassificationDataset):
    def __init__(
        self,
        tokenizer: AutoTokenizer,
        add_space: bool = True,
        ...
        dset = load_dataset("openlifescienceai/mmlu_professional_medicine")

        # Handle MMLU dataset splits - use 'dev' as train, 'validation' as validation, 'test' as test
        if "train" not in dset:
            dset["train"] = dset["dev"]  # Use dev split for training

        prompt = """Answer the following professional medicine question by selecting the correct option.
...
usmle = USMLEDataset
medical_genetics = MedicalGeneticsDataset
professional_medicine = ProfessionalMedicineDataset
